...
export default class GradeControl extends Component {
    ...
    render() {
        ...
        return (
            <Grid>
                <ButtonToolbar>
                    <Switch checked={this.state.editable} label="是否允许修改" onChange={handleBooleanChange((bool)=>this.setState({editable:bool}))} />
                    <Button bsStyle="default" disabled={this.state.dataState!=2 && this.state.dataState!=4} onClick={this.finishEdit}>提交</Button>
                    <Label bsStyle="default">{GradeControl.stateMap[this.state.dataState]}</Label>
                </ButtonToolbar>
