...
import {
    Button, ButtonToolbar, Checkbox, Col, Form, FormControl, FormGroup, Grid, Radio, Row, ToggleButton,
    ToggleButtonGroup
} from 'react-bootstrap';
import ReactDataGrid from "react-data-grid";
// import PropTypes from 'prop-types';
import {SemesterList,ExamType,TeacherList,AutoFormater,ModalDialog,TeacherChooser,CourseList,StudentClassList} from "../components";
import $ from 'jQuery';
...
export default class GradeInput extends Component {
    ...
    calaulateTotalGrade(){
        ...
        rows = rows.map(row=>{
            let finalGrade = GradeInput.calcTotalGrade(row,this.state);
    ...
    render(){
        ...
        return (<Grid fluid={true}>
            ...
            <ButtonToolbar>
                ...
                <Button bsStyle="default" onClick={this.submitGrade.bind(this)}>提交</Button>
                <ToggleButtonGroup type="radio" name="options" defaultValue={0} onChange={checkedN}>
                    {
    ...
    renderButtom (){
        ...
        const courseList = ( <Col key="courseCol" md={4}>
            <Row><Col md={12}><CourseList defaultValue={this.state.teaching_task_id} teacher_id={this.state.teacher_id} exam_type={this.state.exam_type} semester_id={this.state.semester_id}
        ...
        return (<Row>
            {(this.state.input_type === 1)?
                [teacherList,courseList,studentList]:null}
            {(this.state.input_type === 2)?
                [courseList,studentList]:null}
            {(this.state.input_type === 3)?
                [studentList,courseList]:null}
            {
                (
